...
//+------------------------------------------------------------------+
//| Célula de Memoria de Dolor por Contexto                          |
//| Sistema de Endurecimiento Contextual (Inmunológico)              |
//+------------------------------------------------------------------+
struct ContextPainCell
{
    int    totalTrades;
    int    consecutiveLosses;
    double netProfit;
    double hardeningFactor;   // 1.0 = neutro, >1.0 = contexto endurecido
    double worstLoss;         // Peor pérdida registrada en este contexto
    ...
    void Update(const double profit)
    {
        totalTrades++;
        netProfit += profit;
        ...
        if(profit > 0.0)
        {
            // GANANCIA: relajamos lentamente, nunca por debajo de 1.0
            consecutiveLosses = 0;
            hardeningFactor   = MathMax(1.0, hardeningFactor - 0.02);
        }
        else
        {
            // PÉRDIDA: endurecemos más rápido y de forma proporcional
            consecutiveLosses++;

            double pain = 0.08; // penalización base (~8%)

            // Incremento según magnitud de la pérdida
            double magnitude = MathMin(3.0, MathAbs(profit));
            pain += magnitude * 0.03;

            // Bonificación por racha de pérdidas consecutivas
            if(consecutiveLosses > 1)
                pain += consecutiveLosses * 0.03;

            hardeningFactor = MathMin(3.0, hardeningFactor + pain);
    ...
    // Reseteo con "cicatriz": no volvemos a ingenuidad total
    void ConsolidateMemory()
    {
        if(hardeningFactor > 1.0)
        {
            // Conserva el 50% del endurecimiento acumulado
            hardeningFactor = 1.0 + (hardeningFactor - 1.0) * 0.5;
        }
        else
        {
            // Nunca permitimos exceso de confianza estructural (<1.0)
            hardeningFactor = 1.0;
        }
...
// Parámetros de tamaño de la matriz inmunológica
#define ML_SESSION_BUCKETS 5
...
private:
    ...
    // === MATRIZ DE ENDURECIMIENTO CONTEXTUAL (Sistema Inmunológico) ===
    ContextPainCell m_painMatrix[ML_SESSION_BUCKETS][ML_REGIME_BUCKETS][ML_VOL_BUCKETS][2];
    datetime        m_lastConsolidationTime;

    // Estadísticas agregadas por régimen para detectar si el mercado plano (ranging)
    // es la principal fuente de pérdida
    double m_regimeNetProfit[ML_REGIME_BUCKETS];
    int    m_regimeTrades[ML_REGIME_BUCKETS];

    // Parámetros de control para el endurecimiento especial en mercados planos
...
public:
    MetaLearningSystem() {
        ...
        // Inicializar matriz de dolor / endurecimiento contextual
        for(int s = 0; s < ML_SESSION_BUCKETS; s++)
        {
            for(int r = 0; r < ML_REGIME_BUCKETS; r++)
            {
                for(int v = 0; v < ML_VOL_BUCKETS; v++)
                {
                    for(int d = 0; d < 2; d++)
                    {
                        m_painMatrix[s][r][v][d].Initialize();
                    }
                }
            }
        }
        m_lastConsolidationTime = TimeCurrent();

        // Inicializar estadísticas por régimen (para detectar si flat es la mayor pérdida)
        for(int r = 0; r < ML_REGIME_BUCKETS; r++)
        {
            m_regimeNetProfit[r] = 0.0;
            m_regimeTrades[r]    = 0;
        }
...
double GetOverallWinRate() {
    ...
    //=== SISTEMA INMUNOLÓGICO / MATRIZ DE ENDURECIMIENTO CONTEXTUAL ===

    // Mapea la sesión real a índice seguro
    int MapSessionToIndex(const ENUM_MARKET_SESSION session) const
    {
        int idx = (int)session;
        if(idx < 0) idx = 0;
        if(idx >= ML_SESSION_BUCKETS) idx = ML_SESSION_BUCKETS - 1;
        return idx;
    }

    // Mapea el régimen de mercado a índice seguro
    int MapRegimeToIndex(const ENUM_MARKET_REGIME regime) const
    {
        int idx = (int)regime;
        if(idx < 0) idx = 0;
        if(idx >= ML_REGIME_BUCKETS) idx = ML_REGIME_BUCKETS - 1;
        return idx;
    }

    // Mapea volatilidad relativa (ratio) a bucket discreto
    int MapVolatilityToIndex(const double volatility) const
    {
        // Buckets: 0 = ultra baja, 1 = baja, 2 = normal, 3 = alta, 4 = extrema
        if(volatility < 0.7)  return 0;
        if(volatility < 0.9)  return 1;
        if(volatility < 1.5)  return 2;
        if(volatility < 2.5)  return 3;
        return 4;
    }

    // Multiplicador de endurecimiento para un contexto dado.
    // Se usa ANTES de abrir una operación para exigir más fuerza/convicción.
    double GetContextHardeningMultiplier(const ENUM_MARKET_SESSION session,
                                         const ENUM_MARKET_REGIME  regime,
                                         const double              volatility,
                                         const bool                isLong) const
    {
        int sIdx = MapSessionToIndex(session);
        int rIdx = MapRegimeToIndex(regime);
        int vIdx = MapVolatilityToIndex(volatility);
        int dIdx = (isLong ? 0 : 1);

        double factor = m_painMatrix[sIdx][rIdx][vIdx][dIdx].hardeningFactor;

        // --- NUEVO: Cálculo agregado por régimen para detectar si FLAT (RANGING)
        // es la mayor fuente de pérdida reciente, y endurecer aún más en ese caso.
        bool flatIsMajorPain = false;
        int  worstIdx        = -1;
        double worstAvg      = 0.0;

        // Buscar el régimen con peor profit medio
        for(int r = 0; r < ML_REGIME_BUCKETS; r++)
        {
            if(m_regimeTrades[r] < m_minTradesForRegimePainAnalysis)
                continue;

            double avg = m_regimeNetProfit[r] / (double)m_regimeTrades[r];
        ...
        if(worstIdx != -1)
        {
            int flatIdx = MapRegimeToIndex(REGIME_RANGING);
            if(worstIdx == flatIdx)
                flatIsMajorPain = true;
        }

        // Si el mercado plano (ranging) es donde más perdemos,
        // y el contexto actual también es plano, aplicamos un endurecimiento extra.
        if(flatIsMajorPain && regime == REGIME_RANGING)
        {
            // Endurecimiento extra limitado por m_flatExtraHardeningMax
            double extraMult = 1.0;

            // Cuanto más negativa la media, mayor el endurecimiento, con límite.
            if(worstAvg < 0.0)
            {
                // Escala suave: 1.0 a 1.5 aprox según severidad
                double severity = MathMin(1.0, MathAbs(worstAvg));
                extraMult = 1.0 + severity * 0.5; // hasta +50% adicional
            }

            extraMult = MathMin(m_flatExtraHardeningMax, extraMult);
            double oldFactor = factor;
            factor = MathMin(3.0, factor * extraMult);

            PrintFormat("🧱 ML: Mercado PLANO identificado como mayor pérdida. Endurecimiento extra x%.2f (%.2f→%.2f)",
                        extraMult,
                        oldFactor,
                        factor);
        }
        ...
        if(factor > 1.1)
        {
            PrintFormat("🛡️ ML: Zona endurecida S:%d R:%d V:%d D:%d -> x%.2f",
                        sIdx, rIdx, vIdx, dIdx, factor);
        }

        return factor;
    }

    // Registra el resultado de un ciclo completo dentro de la matriz inmunológica.
    // Debe llamarse cuando se cierra la operación (o ciclo) con su profit real.
    void RegisterContextResult(const ENUM_MARKET_SESSION session,
                               const ENUM_MARKET_REGIME  regime,
                               const double              volatility,
                               const bool                isLong,
                               const double              profit)
    {
        ...
        m_painMatrix[sIdx][rIdx][vIdx][dIdx].Update(profit);

        // Actualizar estadísticas agregadas por régimen
        int regIdx = MapRegimeToIndex(regime);
        m_regimeNetProfit[regIdx] += profit;
        m_regimeTrades[regIdx]++;

        CheckPeriodicConsolidation();
    }

    // Consolidación periódica de la memoria (reseteo mensual con cicatrices).
    void CheckPeriodicConsolidation()
    {
        // 30 días ~ 2 592 000 segundos
        if(TimeCurrent() - m_lastConsolidationTime <= 2592000)
            return;

        Print("🧠 ML: Consolidación de memoria mensual (cicatrices de contexto)");
        ...
        for(int s = 0; s < ML_SESSION_BUCKETS; s++)
        {
            for(int r = 0; r < ML_REGIME_BUCKETS; r++)
            {
                for(int v = 0; v < ML_VOL_BUCKETS; v++)
                {
                    for(int d = 0; d < 2; d++)
                    {
                        m_painMatrix[s][r][v][d].ConsolidateMemory();
                    }
                }
            }
        }

        // También podemos suavizar ligeramente las estadísticas por régimen
        for(int r = 0; r < ML_REGIME_BUCKETS; r++)
        {
            // Atenuar memoria (como EMA inversa)
            m_regimeNetProfit[r] *= 0.5;
            m_regimeTrades[r]    = (int)MathFloor(m_regimeTrades[r] * 0.5);
        }
